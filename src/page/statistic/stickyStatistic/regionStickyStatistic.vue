<template>
  <div class="page combine-page">
    <template>
      ...
      <div class="common-selecter">
        <el-select v-model="condiction.regionId" :placeholder="configName.REGION">
          <el-option v-for="item in regionList" :key="item.id" :label="item.name" :value="item.id"></el-option>
        </el-select>
      </div>
      <div class="common-selecter">
        <el-select v-model="condiction.priceSection" :placeholder="configName.MONEYPART">
    ...
    <template>
      <div class="table">
        <el-table ref="OrderTable" v-loading="loading" :data="resultList">
          ...
          <el-table-column prop="priceSection" :label="configName.MONEYPART"></el-table-column>
